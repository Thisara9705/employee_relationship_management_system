...
<?php
    ...
    $en   = new DateTime('yesterday', new DateTimeZone('Asia/Colombo'));
    $end = $en->modify('+1 day');
    ...
    for($i = $begin; $i < $end; $i->modify('+1 day') ){
        ...
        if ($total_present_head == 0 AND $total_ica == 0 AND $total_ac== 0 ) {
            continue;
        }else{
        $total_presents[] =  $total_present_head;
        $total_icas[] =  $total_ica;
        $total_acs[] =  $total_ac;
        $dates[] = $date;
        }
    }
    ...
    for($i = $begin; $i < $end; $i->modify('+1 day') ){
        ...
        $date = $i->format("Y-m-d");
        $all = $db->query("SELECT p_date,total FROM planned_cadre WHERE DATE(p_date) = '$date'");
        $fetch = $all->fetchAll(PDO::FETCH_OBJ);
        $total_planned_cadre=0;

        foreach($fetch as $data){
            $total_planned_cadre = $data->total;  
        }
        if ($total_planned_cadre == 0) {
            continue;
        }else{
            $total_planned_cadres[] =  $total_planned_cadre;
        }
    }
?>
...
<script>
var yArray1 = <?php echo json_encode($total_presents); ?>;
var yArray2 = <?php echo json_encode($total_icas); ?>;
var yArray3 = <?php echo json_encode($total_acs); ?>;
var yArray4 = <?php echo json_encode($total_planned_cadres); ?>;
